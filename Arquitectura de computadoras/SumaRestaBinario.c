...
int main(){
	...
	char binario2[100],binariof2[100]; //cadenas para almacenar el segundo numero a sumar/restar
	int exp1,longf1,exp2,longf2,cont,temp,long1,long2,difexp,sumdec; //para determinar el exponente y longitud de las cadenas 
	int i=0,j=0,numint1,numint2,carry,sum,rest,acum=0; //contadores y variabla para obtener parte entera
...
//cadena 2
	...
	for(i=32;i>=0;i--){
		...
		switch(sum){
			...
			case 3: resum[i]=1; carry=1; break; 
 	     	...
 	     	printf("\n");
 	i=31;
 	while(resum[i]!=1){ //QUITAR LOS CEROS A LA DERECHA
 		resum[i]='\0';
 		acum++;
 		i--;
	 }
	...
	if(resul2[0]==0){ //elimina el cero a la izquiersa en la suma (si la hay)
		for(i=1;i<24;i++){
			resul2[i-1]=resul2[i];
		}
	}	
